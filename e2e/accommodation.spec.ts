...
const LIST_URL = '/travel18/accommodation/list'
const ADD_URL = '/travel18/accommodation/add'
const EDIT_URL = '/travel18/accommodation/edit'
const REMOVE_URL = '/travel18/accommodation/remove'

test.describe.configure({ mode: 'serial' })

const TEST_NAME = `E2E酒店_${Date.now()}`
...
test.describe('住宿管理', () => {
  ...
  test('页面渲染', async ({ loggedInPage }) => {
    ...
    await waitForTableLoad(page, '/accommodation', LIST_URL)
  ...
  test('新增 — 真实创建数据', async ({ loggedInPage }) => {
    ...
    await expect(page.locator('.ant-drawer-title')).toContainText('新增住宿')

    const drawer = page.locator('.ant-drawer')
    drawer.locator('.ant-form-item').filter({ hasText: '酒店名称' }).locator('input').fill(TEST_NAME)
    ...
    const record = listResult.rows?.find((r: any) => r.accommodationName === TEST_NAME)
    testRecordId = record?.accommodationId
  })
  ...
  test('查看详情', async ({ loggedInPage }) => {
    ...
    await expect(page.locator('.ant-drawer-title')).toContainText('住宿详情')
  ...
  test('编辑 — 真实修改数据', async ({ loggedInPage }) => {
    ...
    await expect(page.locator('.ant-drawer-title')).toContainText('编辑住宿')

    const drawer = page.locator('.ant-drawer')
    const nameInput = drawer.locator('.ant-form-item').filter({ hasText: '酒店名称' }).locator('input')
  ...
  test('上下架切换', async ({ loggedInPage }) => {
    const page = loggedInPage
    await waitForTableLoad(page, '/accommodation', LIST_URL)
    // Find the test record row (name was changed by edit test)
  ...
  test('表单必填校验', async ({ loggedInPage }) => {
    ...
    await expect(page.locator('.ant-drawer').getByText('请输入酒店名称')).toBeVisible()
  })
