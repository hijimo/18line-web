...
const LIST_URL = '/travel18/dining/list'
const ADD_URL = '/travel18/dining/add'
const EDIT_URL = '/travel18/dining/edit'
const REMOVE_URL = '/travel18/dining/remove'

test.describe.configure({ mode: 'serial' })

const TEST_NAME = `E2E餐厅_${Date.now()}`
...
test.describe('餐饮管理', () => {
  ...
  test('页面渲染', async ({ loggedInPage }) => {
    ...
    await waitForTableLoad(page, '/dining', LIST_URL)
  ...
  test('新增 — 真实创建数据', async ({ loggedInPage }) => {
    ...
    await expect(page.locator('.ant-drawer-title')).toContainText('新增餐饮')

    const drawer = page.locator('.ant-drawer')
    drawer.locator('.ant-form-item').filter({ hasText: '餐厅名称' }).locator('input').fill(TEST_NAME)
    ...
    const record = listResult.rows?.find((r: any) => r.diningName === TEST_NAME)
    testRecordId = record?.diningId
  })
  ...
  test('查看详情', async ({ loggedInPage }) => {
    ...
    await expect(page.locator('.ant-drawer-title')).toContainText('餐饮详情')
  ...
  test('编辑 — 真实修改数据', async ({ loggedInPage }) => {
    ...
    await expect(page.locator('.ant-drawer-title')).toContainText('编辑餐饮')

    const drawer = page.locator('.ant-drawer')
    const nameInput = drawer.locator('.ant-form-item').filter({ hasText: '餐厅名称' }).locator('input')
  ...
  test('表单必填校验', async ({ loggedInPage }) => {
    ...
    await expect(page.locator('.ant-drawer').getByText('请输入餐厅名称')).toBeVisible()
  })
