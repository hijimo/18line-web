/**
 * Generated by orval v7.17.0 🍺
 * Do not edit manually.
 * 十八线_接口文档
 * 十八线——小众旅游
 * OpenAPI spec version: 版本号:0.0.1
 */
import type {
  AddSave5Params,
  AjaxResult,
  EditSave5Params,
  Export4Params,
  GetInfo9PathParameters,
  List5Params,
  Remove7Params,
  TableDataInfo,
} from '../../../types/api';
import { orvalMutator } from '../../../utils/orval-mutator';

export const get = () => {
  /**
   * @summary 删除打卡点
   */
  const remove7 = (params: Remove7Params) => {
    return orvalMutator<AjaxResult>({ url: `/travel18/checkin/remove`, method: 'POST', params });
  };
  /**
   * @summary 查询打卡点列表
   */
  const list5 = (params: List5Params) => {
    return orvalMutator<TableDataInfo>({ url: `/travel18/checkin/list`, method: 'POST', params });
  };
  /**
   * @summary 导出打卡点列表
   */
  const export4 = (params: Export4Params) => {
    return orvalMutator<void>({ url: `/travel18/checkin/export`, method: 'POST', params });
  };
  /**
   * @summary 修改保存打卡点
   */
  const editSave5 = (params: EditSave5Params) => {
    return orvalMutator<AjaxResult>({ url: `/travel18/checkin/edit`, method: 'POST', params });
  };
  /**
   * @summary 新增保存打卡点
   */
  const addSave5 = (params: AddSave5Params) => {
    return orvalMutator<AjaxResult>({ url: `/travel18/checkin/add`, method: 'POST', params });
  };
  /**
   * @summary 查询打卡点详情
   */
  const getInfo9 = ({ checkinId }: GetInfo9PathParameters) => {
    return orvalMutator<AjaxResult>({ url: `/travel18/checkin/${checkinId}`, method: 'GET' });
  };
  return { remove7, list5, export4, editSave5, addSave5, getInfo9 };
};
export type Remove7Result = NonNullable<Awaited<ReturnType<ReturnType<typeof get>['remove7']>>>;
export type List5Result = NonNullable<Awaited<ReturnType<ReturnType<typeof get>['list5']>>>;
export type Export4Result = NonNullable<Awaited<ReturnType<ReturnType<typeof get>['export4']>>>;
export type EditSave5Result = NonNullable<Awaited<ReturnType<ReturnType<typeof get>['editSave5']>>>;
export type AddSave5Result = NonNullable<Awaited<ReturnType<ReturnType<typeof get>['addSave5']>>>;
export type GetInfo9Result = NonNullable<Awaited<ReturnType<ReturnType<typeof get>['getInfo9']>>>;
