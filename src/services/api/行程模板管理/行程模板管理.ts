...
export const get = () => {
   ...
   * @summary 新增行程模板
   */
  const add1 = (params?: Add1Params) => {
    return orvalMutator<AjaxResult>({ url: `/travel18/template`, method: 'POST', params });
  };
  /**
   * @summary 删除行程模板
   */
  ...
  const generate = (params?: GenerateParams) => {
    return orvalMutator<AjaxResult>({ url: `/travel18/template/generate`, method: 'POST', params });
  };
  /**
   * @summary 修改行程模板
   */
  const edit13 = (params?: Edit13Params) => {
    return orvalMutator<AjaxResult>({ url: `/travel18/template/edit`, method: 'POST', params });
  };
  /**
   * @summary 查询模板详情（含日程明细）
   */
  const getInfo3 = ({ templateId }: GetInfo3PathParameters) => {
