/**
 * Generated by orval v7.17.0 🍺
 * Do not edit manually.
 * 十八线_接口文档
 * 十八线——小众旅游
 * OpenAPI spec version: 版本号:0.0.1
 */
import type {
  Add3Params,
  AjaxResult,
  AuthRolePathParameters,
  ChangeStatusParams,
  DeptTreeParams,
  Edit3Params,
  Export8Params,
  GetInfo15PathParameters,
  ImportDataParams,
  InsertAuthRoleParams,
  List15Params,
  Remove14PathParameters,
  ResetPwdParams,
  TableDataInfo,
} from '../../../types/api';
import { orvalMutator } from '../../../utils/orval-mutator';

export const get = () => {
  /**
   * @summary 修改用户
   */
  const edit3 = (params?: Edit3Params) => {
    return orvalMutator<AjaxResult>({ url: `/system/user`, method: 'PUT', params });
  };
  /**
   * @summary 新增用户
   */
  const add3 = (params?: Add3Params) => {
    return orvalMutator<AjaxResult>({ url: `/system/user`, method: 'POST', params });
  };
  /**
   * @summary 重置密码
   */
  const resetPwd = (params?: ResetPwdParams) => {
    return orvalMutator<AjaxResult>({ url: `/system/user/resetPwd`, method: 'PUT', params });
  };
  /**
   * @summary 状态修改
   */
  const changeStatus = (params?: ChangeStatusParams) => {
    return orvalMutator<AjaxResult>({ url: `/system/user/changeStatus`, method: 'PUT', params });
  };
  /**
   * @summary 用户授权角色
   */
  const insertAuthRole = (params: InsertAuthRoleParams) => {
    return orvalMutator<AjaxResult>({ url: `/system/user/authRole`, method: 'PUT', params });
  };
  /**
   * @summary 下载导入模板
   */
  const importTemplate = () => {
    return orvalMutator<void>({ url: `/system/user/importTemplate`, method: 'POST' });
  };
  /**
   * @summary 导入用户数据
   */
  const importData = (params: ImportDataParams) => {
    return orvalMutator<AjaxResult>({ url: `/system/user/importData`, method: 'POST', params });
  };
  /**
   * @summary 导出用户列表
   */
  const export8 = (params?: Export8Params) => {
    return orvalMutator<void>({ url: `/system/user/export`, method: 'POST', params });
  };
  /**
   * @summary 获取用户列表
   */
  const list15 = (params?: List15Params) => {
    return orvalMutator<TableDataInfo>({ url: `/system/user/list`, method: 'GET', params });
  };
  /**
   * @summary 获取部门树列表
   */
  const deptTree = (params?: DeptTreeParams) => {
    return orvalMutator<AjaxResult>({ url: `/system/user/deptTree`, method: 'GET', params });
  };
  /**
   * @summary 根据用户编号获取授权角色
   */
  const authRole = ({ userId }: AuthRolePathParameters) => {
    return orvalMutator<AjaxResult>({ url: `/system/user/authRole/${userId}`, method: 'GET' });
  };
  /**
   * @summary 根据用户编号获取详细信息
   */
  const getInfo14 = () => {
    return orvalMutator<AjaxResult>({ url: `/system/user/`, method: 'GET' });
  };
  /**
   * @summary 根据用户编号获取详细信息
   */
  const getInfo15 = ({ userId }: GetInfo15PathParameters) => {
    return orvalMutator<AjaxResult>({ url: `/system/user/${userId}`, method: 'GET' });
  };
  /**
   * @summary 删除用户
   */
  const remove14 = ({ userIds }: Remove14PathParameters) => {
    return orvalMutator<AjaxResult>({ url: `/system/user/${userIds}`, method: 'DELETE' });
  };
  return {
    edit3,
    add3,
    resetPwd,
    changeStatus,
    insertAuthRole,
    importTemplate,
    importData,
    export8,
    list15,
    deptTree,
    authRole,
    getInfo14,
    getInfo15,
    remove14,
  };
};
export type Edit3Result = NonNullable<Awaited<ReturnType<ReturnType<typeof get>['edit3']>>>;
export type Add3Result = NonNullable<Awaited<ReturnType<ReturnType<typeof get>['add3']>>>;
export type ResetPwdResult = NonNullable<Awaited<ReturnType<ReturnType<typeof get>['resetPwd']>>>;
export type ChangeStatusResult = NonNullable<
  Awaited<ReturnType<ReturnType<typeof get>['changeStatus']>>
>;
export type InsertAuthRoleResult = NonNullable<
  Awaited<ReturnType<ReturnType<typeof get>['insertAuthRole']>>
>;
export type ImportTemplateResult = NonNullable<
  Awaited<ReturnType<ReturnType<typeof get>['importTemplate']>>
>;
export type ImportDataResult = NonNullable<
  Awaited<ReturnType<ReturnType<typeof get>['importData']>>
>;
export type Export8Result = NonNullable<Awaited<ReturnType<ReturnType<typeof get>['export8']>>>;
export type List15Result = NonNullable<Awaited<ReturnType<ReturnType<typeof get>['list15']>>>;
export type DeptTreeResult = NonNullable<Awaited<ReturnType<ReturnType<typeof get>['deptTree']>>>;
export type AuthRoleResult = NonNullable<Awaited<ReturnType<ReturnType<typeof get>['authRole']>>>;
export type GetInfo14Result = NonNullable<Awaited<ReturnType<ReturnType<typeof get>['getInfo14']>>>;
export type GetInfo15Result = NonNullable<Awaited<ReturnType<ReturnType<typeof get>['getInfo15']>>>;
export type Remove14Result = NonNullable<Awaited<ReturnType<ReturnType<typeof get>['remove14']>>>;
