/**
 * Generated by orval v7.17.0 🍺
 * Do not edit manually.
 * 十八线_接口文档
 * 十八线——小众旅游
 * OpenAPI spec version: 版本号:0.0.1
 */
import type {
  Add4Params,
  AjaxResult,
  AllocatedListParams,
  CancelAuthUserAllParams,
  CancelAuthUserParams,
  ChangeStatus1Params,
  DataScopeParams,
  DeptTree1PathParameters,
  Edit4Params,
  Export9Params,
  GetInfo16PathParameters,
  List16Params,
  Remove15PathParameters,
  SelectAuthUserAllParams,
  TableDataInfo,
  UnallocatedListParams,
} from '../../../types/api';
import { orvalMutator } from '../../../utils/orval-mutator';

export const get = () => {
  /**
   * @summary 修改保存角色
   */
  const edit4 = (params?: Edit4Params) => {
    return orvalMutator<AjaxResult>({ url: `/system/role`, method: 'PUT', params });
  };
  /**
   * @summary 新增角色
   */
  const add4 = (params?: Add4Params) => {
    return orvalMutator<AjaxResult>({ url: `/system/role`, method: 'POST', params });
  };
  /**
   * @summary 修改保存数据权限
   */
  const dataScope = (params?: DataScopeParams) => {
    return orvalMutator<AjaxResult>({ url: `/system/role/dataScope`, method: 'PUT', params });
  };
  /**
   * @summary 状态修改
   */
  const changeStatus1 = (params?: ChangeStatus1Params) => {
    return orvalMutator<AjaxResult>({ url: `/system/role/changeStatus`, method: 'PUT', params });
  };
  /**
   * @summary 批量选择用户授权
   */
  const selectAuthUserAll = (params: SelectAuthUserAllParams) => {
    return orvalMutator<AjaxResult>({
      url: `/system/role/authUser/selectAll`,
      method: 'PUT',
      params,
    });
  };
  /**
   * @summary 取消授权用户
   */
  const cancelAuthUser = (params?: CancelAuthUserParams) => {
    return orvalMutator<AjaxResult>({ url: `/system/role/authUser/cancel`, method: 'PUT', params });
  };
  /**
   * @summary 批量取消授权用户
   */
  const cancelAuthUserAll = (params: CancelAuthUserAllParams) => {
    return orvalMutator<AjaxResult>({
      url: `/system/role/authUser/cancelAll`,
      method: 'PUT',
      params,
    });
  };
  /**
   * @summary 导出角色列表
   */
  const export9 = (params?: Export9Params) => {
    return orvalMutator<void>({ url: `/system/role/export`, method: 'POST', params });
  };
  /**
   * @summary 根据角色编号获取详细信息
   */
  const getInfo16 = ({ roleId }: GetInfo16PathParameters) => {
    return orvalMutator<AjaxResult>({ url: `/system/role/${roleId}`, method: 'GET' });
  };
  /**
   * @summary 获取角色选择框列表
   */
  const optionselect = () => {
    return orvalMutator<AjaxResult>({ url: `/system/role/optionselect`, method: 'GET' });
  };
  /**
   * @summary 获取角色列表
   */
  const list16 = (params?: List16Params) => {
    return orvalMutator<TableDataInfo>({ url: `/system/role/list`, method: 'GET', params });
  };
  /**
   * @summary 获取对应角色部门树列表
   */
  const deptTree1 = ({ roleId }: DeptTree1PathParameters) => {
    return orvalMutator<AjaxResult>({ url: `/system/role/deptTree/${roleId}`, method: 'GET' });
  };
  /**
   * @summary 查询未分配用户角色列表
   */
  const unallocatedList = (params?: UnallocatedListParams) => {
    return orvalMutator<TableDataInfo>({
      url: `/system/role/authUser/unallocatedList`,
      method: 'GET',
      params,
    });
  };
  /**
   * @summary 查询已分配用户角色列表
   */
  const allocatedList = (params?: AllocatedListParams) => {
    return orvalMutator<TableDataInfo>({
      url: `/system/role/authUser/allocatedList`,
      method: 'GET',
      params,
    });
  };
  /**
   * @summary 删除角色
   */
  const remove15 = ({ roleIds }: Remove15PathParameters) => {
    return orvalMutator<AjaxResult>({ url: `/system/role/${roleIds}`, method: 'DELETE' });
  };
  return {
    edit4,
    add4,
    dataScope,
    changeStatus1,
    selectAuthUserAll,
    cancelAuthUser,
    cancelAuthUserAll,
    export9,
    getInfo16,
    optionselect,
    list16,
    deptTree1,
    unallocatedList,
    allocatedList,
    remove15,
  };
};
export type Edit4Result = NonNullable<Awaited<ReturnType<ReturnType<typeof get>['edit4']>>>;
export type Add4Result = NonNullable<Awaited<ReturnType<ReturnType<typeof get>['add4']>>>;
export type DataScopeResult = NonNullable<Awaited<ReturnType<ReturnType<typeof get>['dataScope']>>>;
export type ChangeStatus1Result = NonNullable<
  Awaited<ReturnType<ReturnType<typeof get>['changeStatus1']>>
>;
export type SelectAuthUserAllResult = NonNullable<
  Awaited<ReturnType<ReturnType<typeof get>['selectAuthUserAll']>>
>;
export type CancelAuthUserResult = NonNullable<
  Awaited<ReturnType<ReturnType<typeof get>['cancelAuthUser']>>
>;
export type CancelAuthUserAllResult = NonNullable<
  Awaited<ReturnType<ReturnType<typeof get>['cancelAuthUserAll']>>
>;
export type Export9Result = NonNullable<Awaited<ReturnType<ReturnType<typeof get>['export9']>>>;
export type GetInfo16Result = NonNullable<Awaited<ReturnType<ReturnType<typeof get>['getInfo16']>>>;
export type OptionselectResult = NonNullable<
  Awaited<ReturnType<ReturnType<typeof get>['optionselect']>>
>;
export type List16Result = NonNullable<Awaited<ReturnType<ReturnType<typeof get>['list16']>>>;
export type DeptTree1Result = NonNullable<Awaited<ReturnType<ReturnType<typeof get>['deptTree1']>>>;
export type UnallocatedListResult = NonNullable<
  Awaited<ReturnType<ReturnType<typeof get>['unallocatedList']>>
>;
export type AllocatedListResult = NonNullable<
  Awaited<ReturnType<ReturnType<typeof get>['allocatedList']>>
>;
export type Remove15Result = NonNullable<Awaited<ReturnType<ReturnType<typeof get>['remove15']>>>;
