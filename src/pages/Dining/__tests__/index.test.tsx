...
const handlers = [
  ...
  http.get('*/system/dict/data/type/*', () =>
...
describe('餐饮管理 (Dining) Page', () => {
  ...
  it('renders required dining name field in add form', async () => {
    render(<DiningPage />);
    const user = userEvent.setup();
    const addButton = await screen.findByRole('button', { name: /添加餐饮/ });
    await user.click(addButton);
    ...
    expect(screen.getByText('餐厅名称')).toBeInTheDocument();
  });
});
