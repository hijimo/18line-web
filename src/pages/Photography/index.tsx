...
const Photography: React.FC = () => {
  ...
  const request = useTableRequest(photographyApi.list1 as TODO);
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (currentRecord) {
        await photographyApi.editSave1({
          ...params,
          photographyId: currentRecord.photographyId,
        } as TODO);
        message.success('编辑成功');
      } else {
        await photographyApi.addSave2(params as TODO);
  ...
  const handleToggleStatus = async (record: TODO) => {
    ...
    try {
      await photographyApi.editSave1({
        photographyId: record.photographyId,
        status: newStatus,
      } as TODO);
  ...
  const handleDelete = async (record: TODO) => {
    try {
      await photographyApi.remove3({ ids: record.photographyId } as TODO);
  ...
  const columns = [
    ...
    { title: '名称', dataIndex: 'nickname', ellipsis: true },
    {
      ...
      dataIndex: 'gender',
      search: false,
      valueEnum: Object.fromEntries(Object.entries(GenderLabel).map(([k, v]) => [k, { text: v }])),
    },
      ...
      >
        <Form form={form} layout="vertical">
          <Form.Item
            name="nickname"
            label="跟拍师名称"
            rules={[{ required: true, message: '请输入跟拍师名称' }]}
          >
